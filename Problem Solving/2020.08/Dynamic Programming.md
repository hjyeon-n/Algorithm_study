...
[내리막 길 소스 코드](https://github.com/hjyeon-n/Algorithm_study/blob/master/BOJ/2020.08/Solution_1520.java)

[욕심쟁이 판다](#욕심쟁이-판다)랑 비슷한 문제. 단, 이번에는 최대 경로의 길이를 구하는 것이 아니라 경로의 개수를 구해야 한다.

나에겐 재귀가 너무 어렵다 증말...😭

이 문제에서 주의해야 할 사항은

1. DFS로 풀면 시간초과가 되기 때문에 DP와 함께 풀어야 한다.
2. dp 배열은 방문여부도 따지기 때문에 나올 수 없는 값, 예를 들어 -1로 초기화해야 한다.



풀이과정은

1. map 배열을 입력 받는 데 사용하고 dp 배열을 방문여부와 DP를 하는 데 사용하기 위해 정의한다.
2. dp 배열은 -1로 초기화한다.
3. 시작점이 0, 0이라고 문제에 나와있기 때문에 이 정점에서 DFS를 시작한다.
4. DFS 함수에 들어오고, 만약 현재 좌표값이 도착 지점이라면 경로가 하나 완성된 것이기 때문에 1을 반환한다.
5. 그렇지 않으면 현재 메모해둔 값이 있는지 dp 배열을 확인하고 있으면 그 값을 반환한다.
6. 저장된 값이 없으면 점점 작아지는 값을 찾아 경로를 탐색한다.
7. 재귀 특성상, 도착 점에 도착한 뒤 다시 돌아가는 과정이 있기 때문에 경로를 더해주기 위해 DFS 반환값들을 더해주는 식으로 정의한다.
8. 계산한 값을 메모에 저장한다.
...
[상자넣기 소스 코드](https://github.com/hjyeon-n/Algorithm_study/blob/master/BOJ/2020.08/Solution_1965.java) 📦

[가장 긴 증가하는 부분 수열](https://github.com/hjyeon-n/Algorithm_study/blob/master/BOJ/2020.07/Solution_11053.java)과 똑같은 문제.
