...
[N-Queen 소스 코드](https://github.com/hjyeon-n/Algorithm_study/blob/master/BOJ/2020.08/Solution_9663.java) 👑

아아아... 요즘 이것저것 하는 게 많아서 문제 풀기가 너무너무 귀찮다...😥

체스 룰을 몰라서 먼저 체스 룰부터 배웠다. 퀸은 **상하좌우 + 대각선까지 움직일 수 있고** 정점과 인접한 하나의 정점만 이동할 수 있는 게 아니라 **해당 라인은 전부 이동이 가능**하기 때문에 이 부분을 주의해야 했다.

머리도 안 돌아가고 어떻게 할지 몰라서 체스판이니 2차원 배열로 선언했다가 더 오리무중 상태에 빠졌다...😵

그래서 다른 사람 풀이를 많이 참고했다. 다음에 다시 풀어봐야지 꼭!!!

1. 퀸의 자리가 하나 정해지면 그 행과 열에는 다른 퀸이 올 수가 없고, 다음 행의 어떤 열에 퀸을 놓을 수 있다. 그렇기 때문에 일차원 배열로 선언하고 인덱스는 행, 배열의 값은 열이라고 생각해서 문제에 접근해야 한다.
2. 1열부터 N열까지 돌면서 각 1행 N열에 퀸을 놓았을때 가능한 경우를 확인한다. 이를 위해 DFS 한다.
3. row 행까지는 퀸을 놓았고,  row+1행에 퀸을 놓을수 있는지 확인한다.
4. 1열 부터 N열까지 반복하면서 (row+1, i)에 퀸을 놓을 수 있는 경우가 있는지 확인한다.
5. 만약 row가 끝까지 다다랐을 때, 즉 N 값일 때는 경우의 수를 하나 구한 것이기 때문에 cnt를 하나 증가시켜준다.
...
[알파벳 소스 코드](https://github.com/hjyeon-n/Algorithm_study/tree/master/BOJ/2020.08/Solution_1987)

N-Queen 보다 접근하기도, 푸는 것도 더 쉬웠던 문제. ~~(이게 더 백트래킹 기초 문제 같다)~~

어렵지 않게 풀었고, 처음 풀었던 문제는 시간이 오래 걸려서 다른 사람 풀이를 참고해서 다른 버전으로도 풀었다. ver1이 처음 풀었던 버전, ver2는 수정된 버전.

<br>

문제 조건은 크게 **1.  같은 알파벳이 적힌 칸을 두 번 지날 수 없다.** **2. 최대한 몇 칸을 지날 수 있는지를 구한다.** 정도로 나눌 수 있다.

1번의 경우, DFS 함수 내에서 4가지 방향을 보고 같은 알파벳이 있는지 확인하면 된다.

- ver1의 경우
  1. HashMap 자료구조를 선언 (이름은 alpha라고 하자.)
  2. alpha에 해당 영역의 알파벳이 있는지 확인한 뒤 있으면 다시 그 지역부터 DFS 시작, 아니면 다른 구역 탐색
- ver2의 경우
  1. 알파벳을 넣을 수 있도록 크기가 26인(알파벳 수) 1차원 boolean 배열 선언 (마찬가지로 이름은 alpha라고 하자.)
  2. 인덱스는 정수 값이 들어가야 하기 때문에 아스키 코드를 인덱스 대신에 넣어준다. (예: alpha[map[x] [y] - 'A'] 혹은 alpha[map[x] [y] - 65])
  3. boolean 형태이기 때문에 중복 알파벳이 들어왔는지 확인하기 위해서는 false / true만 판별해 주면 된다.

<br>

2번 조건의 경우, 여러가지 경우의 수가 있기 때문에 백트래킹을 이용해서 어떤 경로로 갔을 때 최대 경로가 나오는지 확인해야 한다.

- ver1의 경우
  1. visited 배열을 선언해서 방문 여부를 따진다.
  2. visited의 값이 false인 경우 방문해서 알파벳의 일치 여부를 따진다.
  3. 탐색이 한 번 끝날 때마다 다시 visited의 값을 false로 초기화 해준다. (백트래킹)
  4. 마찬가지로 hashmap 타입의 alpha에서 해당 알파벳을 제거해 준다.
- ver2의 경우
  1. 아까 선언한 boolean 배열의 alpha가 visited 배열의 역할을 수행해 준다. (즉, ver1의 hashmap + visited)
  2. 탐색이 끝날 때마다 alpha의 값을 false로 초기화한다. (백트래킹)