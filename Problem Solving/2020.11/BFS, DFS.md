...
| 문제 번호                                    | 이름                                              |
| -------------------------------------------- | ------------------------------------------------- |
| [9205](https://www.acmicpc.net/problem/9205) | [맥주 마시면서 걸어가기](#맥주-마시면서-걸어가기) |
...
| 문제 번호                                    | 이름                                      |
| -------------------------------------------- | ----------------------------------------- |
| [2251](https://www.acmicpc.net/problem/2251) | [물통](#물통)                             |
| [1600](https://www.acmicpc.net/problem/1600) | [말이 되고픈 원숭이](#말이-되고픈-원숭이) |
| [5427](https://www.acmicpc.net/problem/5427) | [불](#불)                                 |
...
| 문제 번호                                      | 이름                        |
| ---------------------------------------------- | --------------------------- |
| [9466](https://www.acmicpc.net/problem/9466)   | [텀 프로젝트](#텀-프로젝트) |
| [10451](https://www.acmicpc.net/problem/10451) | [순열 사이클](#순열-사이클) |
...
[순열 사이클 소스 코드](https://github.com/hjyeon-n/Algorithm_study/blob/master/BOJ/2020.11/Solution_10451.java)

사실... 당연히 시간 초과나 틀렸다고 나올 줄 알았는데 맞았습니다가 떴다...🙄

일부러 골라 푸는 건 아닌데 요즘은 DFS 문제만 그것도 사이클을 찾는 문제를 푸는 것 같다. 앞의 텀 프로젝트보다 쉬웠고, 중복 여부를 체크하지 않아도 됐다.

이 문제는 "순열" 키워드에 주목해야 한다. 순열이 직접적으로 사용되는 건 아니지만, 순열 그래프를 그리는 문제이기 때문에 기본적으로 알아둬야 할 게 있다. 바로 **중복 입력이 없다**는 것이다. 그래서 이미 사이클을 이룬 건 다시 생각하지 않아도 된다. 두 가지 버전으로 풀었는데 사실상 같은 코드이다. 풀이는 ver2 중점으로 설명하겠다.

1. 순열 사이클 성격에 따라 하나의 케이스에 대해 방문여부를 나타내는 배열은 한 번만 초기화해 주면 된다.
2. 반복문을 돌면서 아직 방문하지 않은 정점이 있다면 사이클을 이루지 않았다는 뜻이기 때문에 DFS 탐색을 시작하고, 사이클의 시작이 되므로 cnt를 증가시킨다.
3. 사이클의 끝을 발견했을 때 DFS 탐색을 끝내야 하므로 이미 node를 방문했다면 반환해준다.
4. 만약 방문하지 않았다면 방문여부를 표시해 주고 다시 DFS 탐색을 시작한다.
5. 모든 점을 방문했다면, cnt를 출력한다.
