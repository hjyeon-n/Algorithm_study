...
| [경주로 건설](#경주로-건설)                    | [경주로 건설](https://programmers.co.kr/learn/courses/30/lessons/67259?language=java) |
...
[경주로 건설 소스 코드](https://github.com/hjyeon-n/Algorithm_study/blob/master/Programmers/2021.05/%EA%B2%BD%EC%A3%BC%EB%A1%9C%20%EA%B1%B4%EC%84%A4.java)

BFS 문제이다. 보자마자 BFS인 건 알았고 방향이나 비용, 좌표를 저장하는 객체를 선언해서 풀려고 했다.

틀린 접근은 아니었는데 방향 전환에서 문제가 있었다.

나는 방향을 수평과 수직 즉, 0과 1로 정하고 이동 시 반대 방향이 나오면 방향을 새로 갱신한 후 비용을 나타내는 값 new_val에 현재 요금에 600원을 더한 값을 대입하고, 그렇지 않으면 현재 요금에 100원을 더한 값을 대입하는 방식을 사용했다.

하지만 첫 시작부터 코너가 발생하면 값이 답보다 커지는 문제점이 생겼다. 즉, 첫 시작은 -1 같은 나올 수 없는 방법으로 초기화하여 오른쪽이든 아래든 이동할 수 있도록 한 후 탐색을 해야 한다. 이 방법은 다른 사람 풀이를 참고했고 대부분의 사람들이 이런 방식을 사용하는 것 같다.

1. 한 좌표마다 최소 비용을 저장하기 위한 이차원 배열 map을 선언한다. (이 배열이 방문여부도 체크하게 된다.)
2. bfs 탐색을 시작한다.
3. 시작점은 0, 0이고 map[0] [0]에 1을 넣어 방문 여부를 표시한다. 방향은 오른쪽이든 아래든 둘 다 가능하기 때문에 -1로 초기화한다.
4. 큐에서 값을 꺼낸다.
5. 만약 현재 값이 목표 지점이라면 answer에 map의 값과 비교하여 최솟값을 갱신한다.
6. 상하좌우 네 방향을 돌면서 탐색을 시작한다. 이때 범위를 체크하고 board 값이 0일 때 즉 빈칸일 때만 탐색한다.
7. 현재 방향이 -1이거나 nd와 같다면 비용은 100원이기 때문에 new_val에 현재 비용인 fee에 100원을 더한다.
8. 현재 방향이 nd랑 다르다면 코너가 발생한 것이기 때문에 new_val에 현재 비용인 fee에 600원을 더한다.
9. map의 값이 0이라면 아직 방문하지 않은 것이기 때문에 map에 new_val 값을 대입하고 큐에 Node 객체를 삽입한다.
10. map의 값이 new_val보다 크거나 같다면 최소 비용을 구하는 문제이기 때문에 map에 new_val 값을 대입하고 큐에 Node 객체를 삽입한다.