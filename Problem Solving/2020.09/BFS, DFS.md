...
[인구 이동 소스 코드](https://github.com/hjyeon-n/Algorithm_study/blob/master/BOJ/2020.09/Solution_16234.java)

📌 2023.01.08 업데이트

으... 생각보다 너무 오래 걸렸다. 아이디어 자체는 금방 떠올렸는데 엣지 케이스에 어떻게 대응해야 할지 너무 오래 고민했음... 😇

내가 고민했던 부분은 만약 그냥 이중 루프문으로 방문여부만 확인하고 bfs를 돌린다고 하면 이전에 지나쳤지만 연합이 되었을 때 다시 접근을 어떻게 해야 할지 고민했었다.

해결 방법은... 이중루프문을 겉으로 감싸는 while 문을 작성하면 된다...ㅎ 🙄

이미 연합이 된 나라에 대해서는 방문 여부를 그대로 저장하고 갈 줄 알았는데 그게 아니고 다시 방문여부를 싹 초기화해서 bfs 탐색을 진행하고 **더이상 연합을 이룰 수 없을 때까지 이를 반복한다.**

기존 연합으로의 편입이라고 생각하지 않고 각각 새로운 연합이라고 생각해야 문제가 풀리는 듯하다.

<br>

<u>✍ 이전에 작성한 풀이!</u>

모르고 풀었는데 삼성 SW 역량 테스트 기출 문제라고 한다. 어려운 문제는 아니었고, 생각하는 데도 오래 걸리지 않았다.

다만... 딴짓을 너무 많이 해서 구현할 때 시간은 오래 걸렸다.

그리고 내 풀이 방식이 시간이 오래 걸리더라... 다른 사람 풀이를 참고했는데도 영 감이 잘 잡히지 않아서 시간을 줄이는 방법에 대해 더 생각해 봐야겠다.

<br>

국경선을 열지 안 열지는 BFS를 돌면서 각각의 값의 차이를 확인해야 하고 값의 차이가 문제에 주어진 조건에 맞으면 방문하고 큐에 넣었다. BFS가 끝나고 다시 메인으로 돌아갔을 때 모두 방문했는지 확인하고 만약 방문하지 않은 곳이 있다면 이전에 합을 구했던 연합과 다른 연합이기 때문에 sum을 초기화해 다시 연합의 인구 수와 연합의 크기를 세는 방식으로 진행했다. 

1. 반복의 수가 정해져있지 않기 때문에 무한 루프로 while문을 정의한다.
2. 인구 이동 수를 세는 answer을 0으로 초기화 해준다. 
3. visited 배열을 초기화한다.
4. map 전체를 돌면서 방문하지 않은 정점에 대해 BFS 탐색을 시작한다.
5. BFS 함수 안에서 인구 수를 세는 sum은 현재 방문 정점을 더한 값으로 초기화하고, 연합의 크기 역시 현재 국가를 포함해 1로 초기화한다.
6. 탐색을 하면서 현재 정점과 방문할 정점의 값의 차이를 구해 L과 R 사이의 값인지 확인하고, 아직 방문한 정점이 아니라면 방문하고 큐에 삽입한다. 
7. 이때, 연합의 크기가 증가한 것이기 때문에 sum에 인구 수를 더해주고 cnt도 1만큼 증가시킨다.
8. 탐색이 끝나고, 방문한 정점에 대해 인구 이동이 일어났기 때문에 map 값에 sum / cnt를 넣어준다. 그리고 방문 여부를 -1로 별도로 표시한다. (visited가 1일 때만 인구 이동이 일어났다고 표시하기 위해)
9. 방문하지 않은 정점이 있는지 확인하고, 그 정점은 다른 연합이기 때문에 다시 한 번 BFS를 실행시킨다. 이때 BFS를 할 때마다 몇 번 호출했는지 확인하기 위해 stop이라는 변수로 수를 세준다.
10. stop이 N * N과 같으면 모든 정점에 대해 한 번씩 호출했다는 뜻이고, 그 의미는 모두가 다른 연합이라는 뜻이기 때문에 while 반복문을 나오고 만약 stop이 N * N과 같지 않으면, 아직 인구 이동이 더 있을 수 있다는 뜻이므로 answer를 증가시키고 3번부터 과정을 반복한다.

<br>

풀이를 쓰다보니 어디서 시간을 줄일 수 있을지 알 것 같다. 8번의 경우를 생략하고 별도의 리스트에 방문 정점을 넣어 해당 좌표에만 sum / cnt 값을 넣으면 모두 탐색할 필요가 없어서 시간이 조금은 줄어들 것 같다. 시도해 봐야지.

✅ 역시나 시간이 아주 많이 줄어들었다! 전에는 DFS로 했을 때 시간 초과였는데 이젠 DFS가 더 빠른 속도로 해결된다!
...
[Puyo Puyo 소스 코드](https://github.com/hjyeon-n/Algorithm_study/blob/master/BOJ/2020.09/Solution_11559.java) 👾

카카오 기출 문제 [프렌즈4블록](https://github.com/hjyeon-n/Algorithm_study/blob/master/Programmers/2020.09/%ED%94%84%EB%A0%8C%EC%A6%884%EB%B8%94%EB%A1%9D.java)과 비슷한 문제. 단 여기서는 2X2 형태가 아니라 상하좌우로 4개 이상 붙어있으면 터뜨릴 수 있다. 

1. 블록이 없을 때까지 게임을 계속 진행해야 하므로 while문으로 무한루프를 돌린다.
2. 터뜨릴 조건을 만났을 때 바로 터뜨리는 게 아니라 이번 회차에서 다른 블록도 터뜨릴 수 있는지 확인해야 하기 때문에 값을 저장하는 block을 선언한다. 만약 이번 회차가 끝나면 터질 블록이라면 1, 아니면 0으로 값을 설정해서 블록 이동에도 사용할 것이다.
3. 프렌즈4블록과 달리 블록이 빈 부분도 존재하므로 while문 안에서 완전 탐색으로 map을 검사하고 아직 방문하지 않았고, map 값이 '.'이 아니라면 해당 정점에 대해 BFS를 시작한다.
4. BFS 함수에서는 BFS를 위한 큐와 블록을 터뜨릴 조건을 만족했을 때 block의 값을 바꿀 수 있게 list를 초기화한다.
5. 해당 정점과 인접한 정점의 문자가 같고 아직 방문하지 않았다면, 방문 표시를 해주고 큐와 list에 인접 정점의 좌표를 넣어준다. 블록을 터뜨릴 수 있는지 확인하기 위해서 cnt도 1만큼 증가시켜준다. (여기의 cnt는 블록의 개수를 의미)
6. BFS가 끝나면 cnt의 개수가 4개 이상인지 확인하고, 4개 이상이라면 터뜨려야 하기 때문에 list가 빌 때까지 반복문을 돌리며 list의 좌표값에 해당하는 block을 1로 바꿔준다. 또한 터질 블록이 있다는 걸 확인하기 위해 flag를 true로 바꿔준다.
7. 메인으로 돌아와 flag가 false이면 더이상 터질 블록이 없다는 것을 의미하기 때문에 1번의 무한 루프문을 빠져나간다.
8. 만약 flag가 true이면 cnt를 증가시키고(이때 cnt는 연쇄 수를 세는 cnt),  블록을 이동해야 한다.
9. moveBlock은 프렌즈4블록과 똑같다. 밑에서부터 확인해서 값이 0일 때 해당 위치들을 큐에 넣어주고 큐에 다 넣은 뒤 하나씩 빼면서 block의 값이 0인 즉, 터지지 않을 블록을 밑으로 당겨주는 작업을 한다.
10. 모든 작업을 끝낸 뒤 cnt를 출력한다.
