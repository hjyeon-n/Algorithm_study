...
[가장 큰 증가 부분 수열 소스 코드](https://github.com/hjyeon-n/Algorithm_study/blob/master/BOJ/2020.07/Solution_11055.java)

[가장 긴 증가하는 부분 수열](#가장-긴-증가하는-부분-수열-⭐)이랑 유사한 문제. 수열 A가 주어졌을 때, 그 수열의 증가 부분 수열 중에서 합이 가장 큰 것을 구하는 문제이다.

즉, 이번에는 길이가 아니라 합이 중점이 되기 때문에 dp[i] = Math.max(dp[i], dp[j] + 1)에서 1이 아닌 현재 arr 값을 넣어주면 된다.

✔ 점화식은 dp[i] = Math.max(dp[j] + arr[i], dp[i])
...
[퇴사 소스 코드](https://github.com/hjyeon-n/Algorithm_study/blob/master/BOJ/2020.07/Solution_14501.java)

📌2023.01.05 업데이트

[dfs](https://github.com/hjyeon-n/Algorithm_study/blob/master/BOJ/2023.01/Solution_14501.java)로 풀었다. 솔직히 좀 멘붕...ㅎ 백트래킹으로 풀어야겠다는 느낌은 났는데 이중 루프로 풀어도 될라나 싶다가 여러가지 경우의 수를 생각했어야 했다는 점을 간과했다.

약 3년 만에 푼 문제인데 간만에 알고리즘 문제를 푼다지만 아직도 이 문제가 어렵다니 좀 충격적이기도 하다... 😥
